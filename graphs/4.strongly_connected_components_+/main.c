...
void init_graph(graph *g) {
    g->data = malloc((N + 10) * sizeof(vertex));
    int i;
    for (i = 1; i <= N; ++i) {
        g->data[i].status = 0;
        g->data[i].parent = 0;
...
int main() {
    ...
    //read graph from file and make adjacency lists
    int i, j, k, v;
    fscanf(input, "%d", &N);
    init_adj_list(original);
    init_adj_list(inverted);
    for (i = 1; i <= N; ++i) {
        fscanf(input, "%d", &k);
        for (j = 1; j <= k; ++j) {
            fscanf(input, "%d", &v);
            adj_list_insert(original, i, v);
            adj_list_insert(inverted, v, i);
        }
    }
    ...
    //free adjacency lists and graphs
    free_list(original);
    free(original);
    free_list(inverted);
    free(inverted);
